...
The examples below use `lubridate` functions for convenient date or time ranges for pulling data from Wisconet. The `today()` function returns today's date, the `now()` function returns the current date and time. You can also set the start or end date specifically using the format "YYYY-MM-DD" like `start_date = "2026-01-01"`. To specify a start/end time, use the format "YYYY-MM-DD HH:MM:SS" using 24-hour notation, like "2026-01-01 12:00:00" for noon. Times are assumed to be in US Central time unless the `timezone` option has been changed.  If you use a date and don't specify a time, it will return the entire day.
...
Or, use a specific date/time range. Format like "YYYY-MM-DD" or "YYYY-MM-DD HH:MM:SS" (using 24 hour time). If time is ommitted it assumes you want the full day (midnight - midnight local time).
...
Use `get_measures_stations()` to fetch data for a specific set of stations in parallel. Note if you want data from all stations, see the use of `get_measures_all()` below.
